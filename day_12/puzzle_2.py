...
def find_good_connections(small_caves_visited, connections, current_location):
    ...
    cave_visited_twice = [k for k, v in counts.items() if v==3 or v==4] #3! careful here
    good_connections = []
    for c in connections:
        if c[0] == current_location:
            if cave_visited_twice == []: #can still visit a small cave twice
                if c[1] != 'start':
                    if counts['end'] == 0:
                        good_connections.append(c)
                    elif c[1] != 'end':
                        good_connections.append(c)
            else: #already visited a small cave twice
...
if __name__ == "__main__":
    ...
    print_paths(sorted(finished_paths, key=len))
